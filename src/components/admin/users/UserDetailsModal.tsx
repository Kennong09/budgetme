...
const UserDetailsModal: FC<UserDetailsModalProps> = ({
  ...
  const getStatusInfo = (status: string) => {
    switch (status) {
      ...
      case 'suspended':
        return { label: 'Suspended', color: '#dc3545', bgColor: 'rgba(220, 53, 69, 0.1)', icon: 'fa-ban' };
      default:
  ...
  const statusInfo = getStatusInfo(user.status || 'inactive');
            ...
            <div className="px-2 md:px-3 py-2" style={{ background: '#f8f9fa', borderBottom: '1px solid #e9ecef' }}>
              ...
              <div className="block md:hidden">
                <div className="grid grid-cols-2 gap-2">
                  ...
                  <div className="flex items-center gap-1.5 bg-white rounded-lg p-2">
                    <i className={`fas ${user.email_confirmed_at ? 'fa-check-circle text-emerald-500' : 'fa-exclamation-circle text-amber-500'} text-xs`}></i>
                    <div>
                      <p className="text-[8px] text-gray-500 leading-none">Email</p>
                      <p className={`text-[10px] font-bold ${user.email_confirmed_at ? 'text-emerald-500' : 'text-amber-500'}`}>
              ...
              <div className="hidden md:block">
                <div className="row text-center g-2">
                  ...
                  <div className="col-3">
                    <div className="d-flex align-items-center justify-content-center">
                      <i className={`fas ${user.email_confirmed_at ? 'fa-check-circle text-success' : 'fa-exclamation-circle text-warning'} mr-2`}></i>
                      <div className="text-left">
                        <small className="text-muted d-block" style={{ fontSize: '0.7rem', lineHeight: 1 }}>Email</small>
                        <strong style={{ fontSize: '0.8rem' }} className={user.email_confirmed_at ? 'text-success' : 'text-warning'}>
                          {user.email_confirmed_at ? 'Verified' : 'Pending'}
                        </strong>
                      </div>
                    </div>
            ...
            <div className="modal-body py-3" style={{ maxHeight: '50vh', overflowY: 'auto' }}>
              ...
              {activeTab === 'overview' && (
                <div className="row">
                  ...
                  <div className="col-lg-5 text-center mb-3 mb-lg-0">
                    ...
                    <h6 className="font-weight-bold mb-1">{user.user_metadata?.full_name || "Unnamed User"}</h6>
                    ...
                    <div className="d-flex justify-content-center flex-wrap" style={{ gap: '6px' }}>
                      <Badge bg={user.status === 'active' ? 'success' : user.status === 'suspended' ? 'danger' : 'secondary'} style={{ fontSize: '0.7rem' }}>
                  ...
                  <div className="col-lg-7">
                    ...
                    <div className="p-2" style={{ background: '#f8f9fa', borderRadius: '8px', fontSize: '0.85rem' }}>
                      <div className="d-flex justify-content-between py-1" style={{ borderBottom: '1px solid #e9ecef' }}>
                        ...
                        <code style={{ fontSize: '0.75rem' }}>{user.id?.substring(0, 16)}...</code>
                      </div>
                      <div className="d-flex justify-content-between py-1" style={{ borderBottom: '1px solid #e9ecef' }}>
                        <small className="text-muted">Member Since</small>
                        <span style={{ fontSize: '0.8rem' }}>{new Date(user.created_at).toLocaleDateString()}</span>
                      </div>
                      <div className="d-flex justify-content-between py-1" style={{ borderBottom: '1px solid #e9ecef' }}>
                        <small className="text-muted">Last Login</small>
                        <span style={{ fontSize: '0.8rem' }}>{user.last_sign_in_at ? formatDate(user.last_sign_in_at) : 'Never'}</span>
                      </div>
                      <div className="d-flex justify-content-between py-1">
                        <small className="text-muted">Email Verified</small>
                        <span style={{ fontSize: '0.8rem' }}>
                          {user.email_confirmed_at ? (
                            <span className="text-success"><i className="fas fa-check mr-1"></i>{new Date(user.email_confirmed_at).toLocaleDateString()}</span>
                          ) : (
              ...
              {activeTab === 'account' && (
                <div>
                  <div className="row">
                    <div className="col-md-6 mb-3">
                      ...
                      <div className="p-2" style={{ background: '#f8f9fa', borderRadius: '8px' }}>
                        <table className="table table-sm table-borderless mb-0" style={{ fontSize: '0.8rem' }}>
                          <tbody>
                            <tr><td className="text-muted py-1">Full Name</td><td className="text-right py-1"><strong>{user.user_metadata?.full_name || 'Not set'}</strong></td></tr>
                            <tr><td className="text-muted py-1">Email</td><td className="text-right py-1">{user.email}</td></tr>
                            ...
                            <tr><td className="text-muted py-1">Status</td><td className="text-right py-1"><Badge bg={user.status === 'active' ? 'success' : 'secondary'} style={{ fontSize: '0.65rem' }}>{user.status || 'Inactive'}</Badge></td></tr>
                          </tbody>
                        </table>
                      </div>
                    </div>
                    <div className="col-md-6 mb-3">
                      ...
                      <div className="p-2" style={{ background: '#f8f9fa', borderRadius: '8px' }}>
                        <table className="table table-sm table-borderless mb-0" style={{ fontSize: '0.8rem' }}>
                          <tbody>
                            <tr><td className="text-muted py-1">Created</td><td className="text-right py-1">{new Date(user.created_at).toLocaleDateString()}</td></tr>
                            <tr><td className="text-muted py-1">Last Login</td><td className="text-right py-1">{user.last_sign_in_at ? new Date(user.last_sign_in_at).toLocaleDateString() : 'Never'}</td></tr>
                            ...
                            <tr><td className="text-muted py-1">Verified</td><td className="text-right py-1"><Badge bg={user.email_confirmed_at ? 'success' : 'warning'} style={{ fontSize: '0.65rem' }}>{user.email_confirmed_at ? 'Yes' : 'No'}</Badge></td></tr>
                          </tbody>
                        </table>
                      </div>
                    </div>
                  ...
                  {/* Status Management */}
                  <h6 className="text-danger mb-2" style={{ fontSize: '0.9rem' }}><i className="fas fa-user-cog mr-2"></i>Status Management</h6>
                  <div className="p-3" style={{ background: '#f8f9fa', borderRadius: '8px' }}>
                    <p className="text-muted mb-2" style={{ fontSize: '0.8rem' }}>Change user account status:</p>
                    <div className="d-flex flex-wrap" style={{ gap: '8px' }}>
                      ...
                      <button 
                        className={`btn btn-sm ${user.status === 'inactive' ? 'btn-secondary' : 'btn-outline-secondary'}`}
                        onClick={() => onChangeStatus(user, 'inactive')}
                        disabled={loading}
                        style={{ borderRadius: '16px', padding: '4px 12px', fontSize: '0.8rem' }}
                      >
                        <i className="fas fa-pause-circle mr-1"></i>Inactive
                      </button>
                      <button 
                  ...
                  <div className="row g-2">
                    <div className="col-md-4 mb-2">
                      <div 
                        ...
                        onClick={() => onEdit(user)}
                        onMouseEnter={(e) => { e.currentTarget.style.borderColor = '#007bff'; e.currentTarget.style.background = '#e7f1ff'; }}
                        onMouseLeave={(e) => { e.currentTarget.style.borderColor = 'transparent'; e.currentTarget.style.background = '#f8f9fa'; }}
                      >
                        <div className="d-flex align-items-center justify-content-center mb-2" style={{ width: '40px', height: '40px', background: 'rgba(0, 123, 255, 0.1)', borderRadius: '8px', margin: '0 auto' }}>
                          <i className="fas fa-edit text-primary"></i>
                        </div>
                        <h6 className="mb-1" style={{ fontSize: '0.85rem' }}>Edit User</h6>
                        <small className="text-muted" style={{ fontSize: '0.75rem' }}>Modify details & settings</small>
                      </div>
                    </div>
                    <div className="col-md-4 mb-2">
                      <div 
                    ...
                    <div className="col-md-4 mb-2">
                      ...
                      >
                        ...
                        <h6 className="mb-1" style={{ fontSize: '0.85rem' }}>Delete User</h6>
                        <small className="text-muted" style={{ fontSize: '0.75rem' }}>Permanently remove</small>
                      </div>
                    </div>
                  </div>
                </div>
